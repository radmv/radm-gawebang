...
class ProjectController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->hasRole('project_client')) {
            // filtering data projek berdasarkan client id == user id
            $projectsQuery = $projectsQuery->whereHas('owner', function ($query) use ($user) {
    ...
    public function complete_project_store(ProjectApplicant $projectApplicant)
    {
        ...
        DB::transaction(function () use ($projectApplicant) {
            ...
            $validated['user_id'] = $projectApplicant->freelancer->id;
