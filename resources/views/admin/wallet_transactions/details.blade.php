    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="flex flex-col p-10 overflow-hidden bg-white shadow-sm sm:rounded-lg gap-y-5">
                <h3 class="mb-5 text-xl font-bold text-indigo-950">
                    {{ $walletTransaction->type === 'Withdraw' ? 'Withdrawal Details' : 'Topup Details' }}
                </h3>
                <div class="flex flex-row items-center justify-between gap-x-5">
                    ...
                    <div>
                        ...
                        <h3 class="text-xl font-bold text-indigo-950">
                            {{ $walletTransaction->created_at->format('M d, Y') }}
                        </h3>
                    </div>
                    <div class="">
                        <p class="text-sm text-slate-500">User</p>
                        <h3 class="text-xl font-bold text-indigo-950">{{ $walletTransaction->user->name }}</h3>
                    </div>
                </div>
                ...
                @if ($walletTransaction->type === 'Withdraw')
                    ...
                    <div>
                        <h3 class="mb-5 text-xl font-bold text-indigo-950">Send Payment to:</h3>
                        <div class="flex flex-row gap-x-10">
                            <div>
                                <p class="text-sm text-slate-500">Bank</p>
                                <h3 class="text-xl font-bold text-indigo-950">{{ $walletTransaction->bank_name }}</h3>
                            </div>
                            <div>
                    ...
                    @if (!$walletTransaction->is_paid)
                        <h3 class="text-xl font-bold text-indigo-950">Confirm Withdrawal</h3>
                        <form method="POST"
                            action="{{ route('admin.wallet_transactions.update', $walletTransaction->id) }}"
                            enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="mt-4">
                                <x-input-label for="proof" :value="__('proof')" />
                                <x-text-input id="proof" class="block w-full mt-1" type="file" name="proof"
                ...
                @if ($walletTransaction->type === 'Topup')
                    <hr class="my-5">
                    <h3 class="mb-5 text-xl font-bold text-indigo-950">Proof of Topup Payment</h3>
                    <img src="{{ Storage::url($walletTransaction->proof) }}" alt=""
                        class="rounded-2xl object-cover w-[300px] h-[200px] mb-3">
